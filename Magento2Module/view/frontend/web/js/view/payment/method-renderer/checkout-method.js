...
define(
    ...
    function (
        ...
        return Component.extend({
            defaults: {
                template: 'SpellPayment_ExpressCheckout/payment/method/checkout/form'
            },
            configData: (window.checkoutConfig || {}).spellpayment_checkout || {},
            prefferredCountryCode: ko.observable(null),
            ...
            initialize: function () {
                ...
                const detected_country = (quote.billingAddress() || {}).countryId
                    || (quote.shippingAddress() || {}).countryId
                    || null;

                $(document).on('change', 'input[name="payment[method]"]:not(.spell-payment-radio)', function(){
                    $('.payment-method.spell').removeClass('_active');
                    $(this).parents('.payment-method').addClass('_active')
                });

                this.prefferredCountryCode(this.getSelectedCountry(
                    detected_country, this.configData.country_options || []
                ));
            },

            setSpellPaymentContent: function(value) {
                var spellPaymentMethod = value.payment_method;
                $(`input[value=${spellPaymentMethod}`).trigger( 'click' );
                $('.spell-content').addClass('hidden');
                $('.spell-content.' + spellPaymentMethod).removeClass('hidden');
                $('.payment-method').removeClass('_active');
                $('.payment-method.spell').addClass('_active');
            },
            ...
            spell_fillPaymentMethodTemplate: function () {
                const spellFilterPMs = () => {// jscs:ignore jsDoc
                    let selected = this.prefferredCountryCode(),
                ...
                spellFilterPMs(this.prefferredCountryCode());
                quote.billingAddress.subscribe(function (address) {
                    if (address != null) {
                        this.prefferredCountryCode(this.getSelectedCountry(
                            address.countryId, this.configData.country_options || []
                        ));
                    }
            ...
            clickPlaceOrder: function () {
                $("#spellpayment_button").trigger( 'click' );
            },
